...
class ViewController: UIViewController, GADFullScreenContentDelegate {
    ...
    private let banner: GADBannerView = {
        ...
        banner.adUnitID = "ca-app-pub-1006221991146598/7971662622"
    ...
    override func viewDidLoad() {
        ...
        guard let url = URL(string: "https://edit-md.netlify.app/") else {
            return
        }
    ...
    @objc func loadInterstitial() {
        let request = GADRequest()
            GADInterstitialAd.load(
              withAdUnitID: "ca-app-pub-1006221991146598/1296070807", request: request
            ) { (ad, error) in
              if let error = error {
    ...
    override func viewDidLayoutSubviews() {
        ...
        banner.frame = CGRect(x: 0, y:view.frame.size.height-110, width: view.frame.size.width, height: 90).integral
        webView.frame = CGRect(x: 0, y:0, width: view.frame.size.width, height: view.frame.size.height-110)
    }
